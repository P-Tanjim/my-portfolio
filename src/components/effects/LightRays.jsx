...
export default function LightRays({
  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      const resize = () => {
        // DPR=1 + 0.5 render scale = renders at 50% size, CSS upscales
        // For a blurry ray effect this is visually identical but 4x fewer pixels
      ...
      // Timestamp-based 30fps throttle — cleaner than frame-skip, same RAF overhead
      const TARGET_INTERVAL = 1000 / 30;
      let lastDrawTime = 0;
      const loop = (t) => {
        animationIdRef.current = requestAnimationFrame(loop);
        if (t - lastDrawTime < TARGET_INTERVAL) return; // not time to draw yet
        lastDrawTime = t;
        gl.useProgram(program);
